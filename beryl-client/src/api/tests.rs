...
impl MockGateway {
    ...
    fn record_read_layout(&self, ctx: &AttemptContext, req: &beryl_proto::metadata::GetBlockLocationsRequestProto) {
        ...
        let target_data_handle_id = match req.target.as_ref() {
            Some(beryl_proto::metadata::get_block_locations_request_proto::Target::DataHandleId(id)) => Some(id.value),
            _ => None,
        };
...
fn write_handle_proto(handle_id: u64, data_handle_id: u64) -> WriteHandleProto {
    WriteHandleProto {
        handle_id,
        lease_id: Some(beryl_proto::common::LeaseIdProto {
            high: 0,
            low: handle_id,
        }),
        lease_epoch: 1,
        ...
        fencing_token: Some(FencingTokenProto {
            ...
            owner: Some(beryl_types::ClientId::new(7).into()),
            epoch: 1,
        }),
    }
}
...
fn write_target_with_layout(
    ...
    WriteTarget {
        ...
        block_stamp: 1,
        chunk_size: layout.chunk_size,
        block_format_id: beryl_types::BlockFormatId::from_raw(layout.block_format_id).expect("known test block format"),
