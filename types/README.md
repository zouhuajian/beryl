...
`types` owns stable Rust domain and value types shared by Vecton crates. It keeps cross-crate identity, layout, block, lease, and freshness values explicit without choosing runtime policy.
...
The current runtime uses `types` for metadata authority values, worker block/data-plane context, client RPC orchestration, and proto/domain conversion.
