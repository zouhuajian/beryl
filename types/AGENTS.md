...
- file block location, write target, committed block, byte range, fencing token, block stamp, worker epoch, and state watermark values
...
- Keep identity separation explicit: inode identity, data identity, session handles, block identity, route epoch, mount epoch, worker epoch, and state watermark are distinct concepts.
