...
impl<'a> WriteSessionCoordinator<'a> {
    pub(super) fn new(core: &'a FsCore) -> Self {
        Self { core }
    }

    pub(super) async fn execute_release(&self, req: ReleaseSessionInput) -> CoreResult<ReleaseSessionOutput> {
    ...
    pub(super) async fn execute_renew_inode_lease(&self, req: RenewLeaseInput) -> CoreResult<RenewLeaseOutput> {
    ...
    pub(super) async fn execute_open_write(&self, req: OpenWriteInput) -> CoreResult<OpenWriteOutput> {
    ...
    pub(super) async fn execute_close_write(&self, req: CloseWriteInput) -> CoreResult<CloseWriteOutput> {
        let caller_ctx = &req.ctx.caller;
        let file_handle = req.file_handle;
    ...
    pub(super) async fn execute_fsync(&self, req: FsyncBarrierInput) -> CoreResult<FsyncBarrierOutput> {
        let caller_ctx = &req.ctx.caller;
        let inode_id = req.inode_id;
