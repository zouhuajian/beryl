...
use std::sync::Arc;
use tracing::warn;
use types::fs::{Extent, FileAttrs, FsErrorCode, Inode, InodeData, InodeId};
use types::ids::{DataHandleId, LeaseId, MountId, WorkerId};
...
impl AppRaftStateMachine {
    ...
    fn truncate_layout_to_size(
        ...
        new_size: u64,
    ) -> MetadataResult<Vec<types::fs::Extent>> {
...
pub(crate) mod tests {
    ...
    pub(crate) use types::fs::{FileAttrs, Inode};
    pub(crate) use types::ids::{BlockId, BlockIndex, ClientId, DataHandleId, MountId, WorkerId};
    pub(crate) use types::layout::FileLayout;
    pub(crate) use types::CallId;
    ...
    pub(crate) fn lease_id_for_inode_epoch(inode_id: InodeId, lease_epoch: u64) -> types::ids::LeaseId {
        types::ids::LeaseId::new((inode_id.as_raw() as u128) << 64 | (lease_epoch as u128))
    }

    pub(crate) fn extent(block_id: BlockId, file_offset: u64, len: u64) -> types::fs::Extent {
        types::fs::Extent {
            file_offset,
            block_id,
            block_offset: 0,
