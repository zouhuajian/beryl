...
| `destructive_gate.rs` | CRATE_PRIVATE_KEEP_BUT_MOVE | 已收窄为 crate-private；后续可归入 `maintenance/safety`。 |
| `inflight_registry.rs` | CRATE_PRIVATE_KEEP_BUT_MOVE | 已收窄为 crate-private；后续可归入 `maintenance/safety`。 |
...
| `metrics.rs` | REDUCED_IN_P5 | 删除无 updater 的零值导出；保留真实 runtime updater 的共享导出。后续可按 fs/worker/maintenance/authz 拆分。 |
| `maintenance/repair/metrics.rs` | MOVED_IN_P5 | repair queue/task metrics 归入 maintenance repair owner。 |
