...
flowchart TB
    ...
    WorkerRt --> Repair["maintenance repair runtime state"]
...
| 历史残留 | `report_presence` proto RPC 入口仍存在但返回 deprecated/NotSupported、`MemoryStateStore` route-epoch 测试 helper、`MetadataWorkerServiceImpl` 仍有 block-report/dead-worker repair signal injection 到 maintenance repair state。 |
...
| `build_worker_runtime()` | 创建 required `WorkerRuntime`、`WorkerManager`、maintenance repair/orphan queue、planner，并构造 `MetadataWorkerServiceImpl`。 | 已实现 |
...
| `build_worker_background()` | 给 worker service 注入 `WorkerCommandRouter`，router 聚合 maintenance/delete 与 maintenance/repair command source，启动 worker background tasks。 | 已接入 |
...
- leader 对新增 block 做 orphan 检测和 replication planning；这仍是 worker service 内的 transitional repair signal injection，不应描述为完整 maintenance ownership closure。
...
- P4.5 边界校准已修正 ARCHITECTURE/README 的 command router、P1-P4 状态和 repair/delete 路径漂移；runtime 中 repair state 命名改为 maintenance repair state。
...
- `MetadataWorkerServiceImpl` 仍直接持有 maintenance repair queue/planner/orphan queue，用于 block report 和 dead-worker 信号注入；command polling/ack 已经通过 `WorkerCommandRouter`，但 repair ownership 进一步收敛不是本轮内容。
- `metadata/src/maintenance/mod.rs` 仍是 maintenance public re-export 聚合入口；本轮只清理 stale wording，不收窄 export surface。

验证状态：

- P1-P4.5 cleanup 已确认 `cargo fmt --all --check`、`cargo test -p metadata --all-targets`、`cargo clippy -p metadata --all-targets -- -D warnings`、`git diff --check` 和 `cargo test --workspace --all-targets` 当前通过。
...
25. `metadata/src/maintenance/service.rs`
26. `metadata/src/maintenance/gc.rs`
27. `metadata/src/maintenance/orphan.rs`
28. `metadata/src/maintenance/overrep.rs`
