...
impl From<beryl_common::CommonError> for UfsError {
    fn from(err: beryl_common::CommonError) -> Self {
        use beryl_common::CommonErrorKind;
        match err.kind {
            CommonErrorKind::Overloaded => UfsError::Overloaded(err.message),
            CommonErrorKind::Timeout => UfsError::Internal(format!("Timeout: {}", err.message)),
            _ => UfsError::Internal(err.message),
        }
    }
}

impl From<UfsError> for beryl_common::CommonError {
    fn from(err: UfsError) -> Self {
        use beryl_common::CommonErrorKind;
        match &err {
            UfsError::NotFound(_) => beryl_common::CommonError::new(CommonErrorKind::NotFound, err.to_string()),
            UfsError::PermissionDenied(_) => {
                beryl_common::CommonError::new(CommonErrorKind::PermissionDenied, err.to_string())
            }
            UfsError::Overloaded(_) => beryl_common::CommonError::new(CommonErrorKind::Overloaded, err.to_string()),
            UfsError::InvalidPath(_) | UfsError::InvalidSpec(_) => {
                beryl_common::CommonError::new(CommonErrorKind::InvalidArgument, err.to_string())
            }
            UfsError::Unsupported(_) | UfsError::NotImplemented(_) => {
                beryl_common::CommonError::new(CommonErrorKind::Internal, err.to_string())
            }
