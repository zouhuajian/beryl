...
async fn low_target_limits_reject_before_raft_and_release_on_abort() {
    ...
    let per_session = add_block(&mut metadata, first_handle, Some(first_block), 701)
        .await
    ...
    let reopened_target = add_block(&mut metadata, reopened_handle, None, 701)
        .await
    ...
    let replay = add_block(&mut metadata, reopened_handle, None, 701)
        .await
    ...
    let third_target = add_block(&mut metadata, third_handle, None, 701)
        .await
...
async fn add_block(
    ...
    client_id: u64,
) -> Result<WriteTargetProto, RpcErrorDetail> {
    let response = metadata
        ...
        .into_inner();
    let header = response.header.expect("AddBlock response header");
    match header.error {
        Some(error) => Err(rpc_error_from_proto(&error)),
        None => Ok(response.target.expect("successful AddBlock target")),
    }
}
