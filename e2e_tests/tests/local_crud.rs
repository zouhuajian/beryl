...
use bytes::Bytes;
use client::{ClientResult, CreateOptions, InodeKind, ListOptions};
use e2e_tests::{data::deterministic_bytes, TestCluster};
...
async fn local_client_crud_roundtrip() {
    ...
    let suffix = Bytes::from_static(b"-vecton-append-suffix");
