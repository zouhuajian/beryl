...
impl From<UfsError> for common::CommonError {
    fn from(err: UfsError) -> Self {
        use common::CommonErrorKind;
        match &err {
            UfsError::NotFound(_) => common::CommonError::new(CommonErrorKind::NotFound, err.to_string()),
            UfsError::PermissionDenied(_) => {
                common::CommonError::new(CommonErrorKind::PermissionDenied, err.to_string())
            }
            UfsError::Overloaded(_) => common::CommonError::new(CommonErrorKind::Overloaded, err.to_string()),
            UfsError::InvalidPath(_) | UfsError::InvalidSpec(_) => {
                common::CommonError::new(CommonErrorKind::InvalidArgument, err.to_string())
            }
            UfsError::Unsupported(_) | UfsError::NotImplemented(_) => {
                common::CommonError::new(CommonErrorKind::Internal, err.to_string())
            }
