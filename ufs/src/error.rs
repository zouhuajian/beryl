...
impl From<common::CommonError> for UfsError {
    fn from(err: common::CommonError) -> Self {
        ...
        match err.code {
            ErrorCode::Overloaded => UfsError::Overloaded(err.message),
            ErrorCode::Timeout => UfsError::Internal(format!("Timeout: {}", err.message)),
...
impl From<UfsError> for common::CommonError {
    fn from(err: UfsError) -> Self {
        use common::ErrorCode;
        match &err {
            UfsError::NotFound(_) => common::CommonError::new(ErrorCode::NotFound, err.to_string()),
            UfsError::PermissionDenied(_) => common::CommonError::new(ErrorCode::PermissionDenied, err.to_string()),
            UfsError::Overloaded(_) => common::CommonError::new(ErrorCode::Overloaded, err.to_string()),
            UfsError::InvalidPath(_) | UfsError::InvalidSpec(_) => {
                common::CommonError::new(ErrorCode::InvalidArgument, err.to_string())
            }
            UfsError::Unsupported(_) | UfsError::NotImplemented(_) => {
                common::CommonError::new(ErrorCode::Internal, err.to_string())
            }
