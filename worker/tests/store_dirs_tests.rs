...
use types::{BlockFormatId, GroupName, Tier, TierFree};
use worker::config::StoreDirConfig;
use worker::store::block::{ChecksumKind, CreateStagingBlockRequest, LocalBlockStore, PublishReadyRequest};
...
fn create_failure_releases_pending_reservation() {
    ...
    let raw_store =
        worker::store::block::FullBlockFileStore::new(worker::store::block::FullBlockFileStoreConfig::new(path));
