...
mod tests {
    ...
    // ========== Service layer read/write tests ==========

    #[tokio::test]
    async fn test_service_read_chunk() {
        ...
        let audit_logger = Arc::new(AuditLogger::new(&audit_dir).unwrap());
        let service = WorkerDataServiceImpl::new(block_store.clone(), audit_logger, layout, WorkerId::new(1), 0);
        ...
        // Write chunk first
        let test_data = Bytes::from(vec![42u8; layout.chunk_size as usize]);
        let chunk_ref = ChunkRef::new(block_id, chunk_idx.as_raw());
        let mut stream = tokio::io::BufReader::new(std::io::Cursor::new(test_data.clone()));
        block_store
            .write_chunk_stream(group_id, chunk_ref, &mut stream)
            .await
            .unwrap();
        ...
        let proto_chunk_id = proto::common::ChunkIdProto {
            block: Some(ProtoBlockId {
                data_handle_id: data_handle_id.as_raw(),
                block_index: block_id.index.as_raw(),
            }),
            chunk_index: chunk_idx.as_raw(),
        };

        let read_req = ReadChunkRequestProto {
            chunk: Some(proto_chunk_id),
            offset_in_chunk: 0,
            len: layout.chunk_size,
            expected_version: 0,
        ...
        let response = service.read_chunk(Request::new(read_req)).await.unwrap();
        let response_inner = response.into_inner();

        assert!(response_inner.data.is_some());
        let chunk_data = response_inner.data.unwrap();
        assert_eq!(chunk_data.data, test_data.to_vec());
    }

    #[tokio::test]
    async fn test_service_write_chunk() {
        use crate::service::WorkerDataServiceImpl;
        use common::audit::AuditLogger;
        use proto::common::{BlockIdProto as ProtoBlockId, FencingTokenProto, RequestHeaderProto};
        use proto::worker::worker_data_service_server::WorkerDataService;
        use proto::worker::{
            ChunkDataProto as ProtoChunkData, ChunkSliceProto as ProtoChunkSlice, WriteChunkRequestProto,
        };
        use std::collections::HashMap;
        use tonic::Request;
        use types::ids::WorkerId;
        ...
        // Write via service
        let test_data = vec![99u8; layout.chunk_size as usize];
        let proto_chunk_id = proto::common::ChunkIdProto {
            block: Some(ProtoBlockId {
                data_handle_id: data_handle_id.as_raw(),
                block_index: block_id.index.as_raw(),
        ...
        let proto_chunk_slice = ProtoChunkSlice {
            chunk: Some(proto_chunk_id.clone()),
            offset_in_chunk: 0,
            len: layout.chunk_size,
        };

        let chunk_data = ProtoChunkData {
            slice: Some(proto_chunk_slice),
            data: test_data.clone().into(),
            checksum32: 0,
        };

        let write_req = WriteChunkRequestProto {
        ...
        let response = service.write_chunk(Request::new(write_req)).await.unwrap();
        let response_inner = response.into_inner();

        assert!(response_inner.stored);

        // Verify data was written
        let read_data = block_store.read_chunk(group_id, block_id, chunk_idx).await.unwrap();
        assert!(read_data.is_some());
        assert_eq!(read_data.unwrap().to_vec(), test_data);
    }

    #[tokio::test]
    async fn test_service_read_chunk_with_ufs_fill() {
        use crate::service::WorkerDataServiceImpl;
        use common::audit::AuditLogger;
        use proto::common::ChunkIdProto as ProtoChunkId;
        use proto::common::RequestHeaderProto;
        ...
        let ufs_file_path = ufs_root.join(data_handle_id.as_raw().to_string());
        let mut ufs_file = std::fs::File::create(&ufs_file_path).unwrap();
        let test_data = vec![77u8; layout.chunk_size as usize];
        ufs_file.write_all(&test_data).unwrap();
        ufs_file.sync_all().unwrap();
        ...
        // Create UfsFiller
        let ufs_filler = Arc::new(UfsFiller::new(
            ufs_registry,
            block_store.clone(),
            audit_logger.clone(),
            layout,
        ...
        ufs_filler.init_limiters(10);

        // Create service with UFS filler
        let service = WorkerDataServiceImpl::with_ufs_filler(
            block_store.clone(),
            audit_logger,
            layout,
            WorkerId::new(1),
        ...
        // Read via service (should fill from UFS)
        use proto::common::BlockIdProto as ProtoBlockId;
        let proto_chunk_id = ProtoChunkId {
            block: Some(ProtoBlockId {
                data_handle_id: data_handle_id.as_raw(),
                block_index: block_id.index.as_raw(),
            }),
        ...
        assert!(response_inner.data.is_some());
        let chunk_data = response_inner.data.unwrap();
        assert_eq!(chunk_data.data, test_data);
    ...
    #[tokio::test]
    async fn test_service_read_range() {
        use crate::service::WorkerDataServiceImpl;
        use common::audit::AuditLogger;
        use futures::StreamExt;
        use proto::common::{ByteRangeProto as ProtoByteRange, CallerContextProto, RequestHeaderProto};
        use proto::worker::worker_data_service_server::WorkerDataService;
        use proto::worker::ReadRangeRequestProto;
        use std::collections::HashMap;
        use tonic::Request;
        use types::ids::WorkerId;
        ...
        let mut expected_data = Vec::new();
        for chunk_idx in 0..num_chunks {
            let chunk_data = vec![(chunk_idx * 10) as u8; layout.chunk_size as usize];
            expected_data.extend_from_slice(&chunk_data);
            let chunk_ref = ChunkRef::new(block_id, ChunkIndex::new(chunk_idx).as_raw());
            let mut stream = tokio::io::BufReader::new(std::io::Cursor::new(Bytes::from(chunk_data)));
            block_store
                .write_chunk_stream(group_id, chunk_ref, &mut stream)
                .await
                .unwrap();
        ...
        // Read range via service
        let range_start = layout.chunk_size / 2;
        let range_len = layout.chunk_size * 2;

        let read_req = ReadRangeRequestProto {
            data_handle_id: data_handle_id.as_raw(),
            range: Some(ProtoByteRange {
                offset: range_start as u64,
                len: range_len as u32,
            }),
            prefer_worker_ids: vec![],
            expected_version: 0,
            read_mode: proto::common::ReadModeProto::ReadModeUnspecified as i32,
        };

        let mut stream = service.read_range(Request::new(read_req)).await.unwrap().into_inner();
        let mut read_data = Vec::new();

        while let Some(chunk_result) = stream.next().await {
            let chunk = chunk_result.unwrap();
            if let Some(data) = chunk.data {
                read_data.extend_from_slice(&data.data);
            }
        }

        // Verify data matches expected range
        let start = range_start as usize;
        let end = (start + range_len as usize).min(expected_data.len());
        assert_eq!(read_data, expected_data[start..end]);
    }

    #[tokio::test]
    async fn test_service_read_range_with_ufs_fill() {
        use crate::service::WorkerDataServiceImpl;
        use common::audit::AuditLogger;
        use futures::StreamExt;
        use proto::common::{ByteRangeProto as ProtoByteRange, CallerContextProto, RequestHeaderProto};
        ...
        // Create test file in UFS (multiple chunks)
        let data_handle_id = DataHandleId::new(1);
        let ufs_file_path = ufs_root.join(data_handle_id.as_raw().to_string());
        let mut ufs_file = std::fs::File::create(&ufs_file_path).unwrap();
        let num_chunks = 3;
        ...
        for chunk_idx in 0..num_chunks {
            let chunk_data = vec![(chunk_idx * 20) as u8; layout.chunk_size as usize];
        ...
        // Read range via service (should fill from UFS)
        let range_start = layout.chunk_size / 2;
        let range_len = layout.chunk_size * 2;
        ...
        // Verify data matches expected range
        let start = range_start as usize;
        let end = (start + range_len as usize).min(test_data.len());
        assert_eq!(read_data, test_data[start..end]);

        // Verify chunks were filled to BlockStore
        for chunk_idx in 0..num_chunks {
            assert!(block_store.has_chunk(group_id, block_id, ChunkIndex::new(chunk_idx)));
        }
    }
    ...
    // ============================================================================
    // Stream operations and block_stamp validation tests
    // ============================================================================

    #[tokio::test]
    async fn test_open_read_stream_block_stamp_mismatch() {
        use crate::service::WorkerDataServiceImpl;
        use common::audit::AuditLogger;
        use proto::common::{BlockIdProto as ProtoBlockId, ByteRangeProto as ProtoByteRange};
        use proto::worker::worker_data_service_server::WorkerDataService;
        use proto::worker::OpenReadStreamRequestProto;
        use tonic::Request;
        use types::ids::WorkerId;
        ...
        // Create a block with block_stamp = 100
        let data_handle_id = DataHandleId::new(1);
        let block_id = BlockId::new(data_handle_id, BlockIndex::new(0));
        let group_id = ShardGroupId::new(0);

        // Write a chunk to create the block
        let chunk_idx = ChunkIndex::new(0);
        let chunk_ref = ChunkRef::new(block_id, chunk_idx.as_raw());
        let data = Bytes::from(vec![1u8; 1024]);
        let mut stream = tokio::io::BufReader::new(std::io::Cursor::new(data));
        block_store
            .write_chunk_stream(group_id, chunk_ref, &mut stream)
            .await
            .unwrap();
        ...
        // Update block_stamp
        block_store.update_block_stamp(group_id, block_id, 100).unwrap();

        let audit_logger = Arc::new(AuditLogger::new(&audit_dir).unwrap());
        let service = WorkerDataServiceImpl::new(block_store.clone(), audit_logger, layout, WorkerId::new(1), 0);

        // Try to open stream with wrong block_stamp
        let req = OpenReadStreamRequestProto {
            ...
            range_in_block: Some(ProtoByteRange { offset: 0, len: 1024 }),
            expected_block_stamp: 50, // Wrong stamp
            preferred_chunk_size: 4096,
        };

        let response = service.open_read_stream(Request::new(req)).await.unwrap();
        let resp = response.into_inner();

        // Should return NEED_REFRESH with STALE reason
        ...
        assert_eq!(resp.current_block_stamp, 100); // Should return current stamp
        assert_eq!(resp.committed_length, 1024);
    }

    #[tokio::test]
    async fn test_open_read_stream_block_not_found() {
        use crate::service::WorkerDataServiceImpl;
        use common::audit::AuditLogger;
        use proto::common::{BlockIdProto as ProtoBlockId, ByteRangeProto as ProtoByteRange};
        use proto::worker::worker_data_service_server::WorkerDataService;
        ...
        // Try to open stream for non-existent block
        let data_handle_id = DataHandleId::new(999);
        let block_id = BlockId::new(data_handle_id, BlockIndex::new(999));
        let req = OpenReadStreamRequestProto {
            header: Some(proto::worker::DataRequestHeaderProto {
                client: Some(proto::common::ClientInfoProto {
                    call_id: "test-call-id".to_string(),
            ...
            range_in_block: Some(ProtoByteRange { offset: 0, len: 1024 }),
            expected_block_stamp: 0, // No validation
            preferred_chunk_size: 4096,
        };
        ...
        // Should return NEED_REFRESH with MOVED reason
        assert!(resp.header.is_some());
        let header = resp.header.unwrap();
        assert!(header.error.is_some());
        let error = header.error.unwrap();
        ...
        assert_eq!(
            error.refresh_reason(),
            proto::common::RefreshReasonProto::RefreshReasonMoved
        );
    }

    #[tokio::test]
    async fn test_open_read_stream_success() {
        use crate::service::WorkerDataServiceImpl;
        use common::audit::AuditLogger;
        use proto::common::{BlockIdProto as ProtoBlockId, ByteRangeProto as ProtoByteRange};
        use proto::worker::worker_data_service_server::WorkerDataService;
        ...
        // Open stream with correct block_stamp
        let req = OpenReadStreamRequestProto {
            header: Some(proto::worker::DataRequestHeaderProto {
                client: Some(proto::common::ClientInfoProto {
                    call_id: "test-call-id".to_string(),
            ...
            range_in_block: Some(ProtoByteRange { offset: 0, len: 1024 }),
            expected_block_stamp: 100, // Correct stamp
            preferred_chunk_size: 4096,
        };
        ...
        assert!(resp.header.is_some());
        let header = resp.header.unwrap();
        // Success: error should be None or error_class = OK
        if let Some(ref error) = header.error {
            assert_eq!(error.error_class(), proto::common::ErrorClassProto::ErrorClassOk);
        }
        assert!(resp.stream_id.is_some());
        assert_eq!(resp.chunk_size, 4096);
        assert_eq!(resp.current_block_stamp, 100);
        assert_eq!(resp.committed_length, 1024);
    }
    ...
    mod commit_write_tests_inline {
        use super::*;
        use crate::service::WorkerDataServiceImpl;
        use common::audit::AuditLogger;
        use proto::common::FencingTokenProto;
        use proto::worker::worker_data_service_server::WorkerDataService;
        use proto::worker::{CommitWriteRequestProto, OpenWriteStreamRequestProto};
        use tonic::Request;
        use types::ids::WorkerId;
        use types::layout::FileLayout;

        async fn make_service() -> WorkerDataServiceImpl {
            let layout = FileLayout::new(4 * 1024 * 1024, 1024 * 1024, 3);
            ...
            std::fs::create_dir_all(&audit_dir).unwrap();
            let audit_logger = Arc::new(AuditLogger::new(&audit_dir).unwrap());
            WorkerDataServiceImpl::new(block_store, audit_logger, layout, WorkerId::new(1), 1)
        }

        fn make_client_header() -> proto::worker::DataRequestHeaderProto {
            proto::worker::DataRequestHeaderProto {
                client: Some(proto::common::ClientInfoProto {
        ...
        #[tokio::test]
        async fn commit_write_success_after_stream() {
            let service: WorkerDataServiceImpl = make_service().await;
            let block_id = BlockId::new(DataHandleId::new(1), BlockIndex::new(0));
            let stream_id = types::ids::StreamId::new(1);
            // Manually register a stream state since block placement isn't set up in this unit test.
            let mut stream_state = crate::stream_manager::StreamState::new(
                stream_id,
                block_id,
                crate::stream_manager::StreamMode::Write,
                None,
                4096,
                4096 * 2,
                0,
            ...
            stream_state.lease_epoch = Some(1);
            stream_state.fencing_owner = Some(types::ids::ClientId::new(123));
            let stream_manager = service.stream_manager_for_test();
            stream_manager.register(stream_state).await;
            stream_manager.update_persisted(stream_id, 4).await;

            let commit_req = CommitWriteRequestProto {
                header: Some(make_client_header()),
            ...
            let resp = service
                .commit_write(Request::new(commit_req))
                .await
                .unwrap()
                .into_inner();
            assert!(resp.header.as_ref().unwrap().error.is_none());
            assert_eq!(resp.committed_length, 4);
        }

        #[tokio::test]
        async fn commit_write_worker_epoch_mismatch() {
            let service: WorkerDataServiceImpl = make_service().await;
            let block_id = BlockId::new(DataHandleId::new(2), BlockIndex::new(0));
            let commit_req = CommitWriteRequestProto {
                header: Some(make_client_header()),
                block_id: Some(proto::common::BlockIdProto {
                    data_handle_id: block_id.data_handle_id.as_raw(),
            ...
            assert_eq!(
                err.refresh_reason,
                proto::common::RefreshReasonProto::RefreshReasonWorkerEpochMismatch as i32
            );
        }

        #[tokio::test]
        async fn commit_write_missing_stream_need_refresh() {
            let service: WorkerDataServiceImpl = make_service().await;
            let block_id = BlockId::new(DataHandleId::new(3), BlockIndex::new(0));
            let commit_req = CommitWriteRequestProto {
                header: Some(make_client_header()),
                block_id: Some(proto::common::BlockIdProto {
                    data_handle_id: block_id.data_handle_id.as_raw(),
            ...
            assert_eq!(
                err.refresh_reason,
                proto::common::RefreshReasonProto::RefreshReasonStaleState as i32
            );
        }
    }
}
