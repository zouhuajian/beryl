...
| `BlockFormat` | `metadata` selects, `worker` persists | Vecton block data/meta interpretation format. Workers persist full `FileLayout.block_size` in `BlockMeta.format.block_size`, persist the selected chunk size and format id, and publish tail or bounded valid length in `BlockMeta.source.effective_len`. Recovery and local reads interpret blocks from persisted `BlockMeta`, not runtime defaults. |
