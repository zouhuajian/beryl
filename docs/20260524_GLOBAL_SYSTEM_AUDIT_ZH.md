...
1. 在 PR-5 register 契约基础上，继续统一 MetadataWorkerService heartbeat/block report 错误契约，删除 allowlist 或收窄到真正 transport failure。
...
2. 做 worker lifecycle 后续 PR：在已实现 startup register 后继续接入 heartbeat、full/delta block report、command ack。
...
- 主要风险：MetadataWorkerService register 已收敛为 structured header error；heartbeat/block report 与 worker streaming path 的错误分类仍需继续收敛。
...
主要优化区域包括：worker heartbeat/block report/command ack lifecycle、worker 公开入口防御性校验、MetadataWorkerService 后续错误契约、proto active surface 状态、真实 E2E 测试。

主要 TODO 区域包括：metadata Raft network、worker heartbeat/block report/command ack 生产 loop、io_uring/SPDK/QUIC/RDMA、部分 maintenance fencing、UFS backend integration profile。
